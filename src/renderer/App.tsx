...
import { CssBaseline, AppBar, Tabs, Tab, Box, CircularProgress, Alert, List, ListItem, ListItemText, Chip, IconButton, Paper, Typography } from '@mui/material';
...
export const App: React.FC = () => {
  ...
  const [activeView, setActiveView] = useState<'todos' | 'stats' | 'calendar'>('todos');
  ...
  const addTodo = (todoData: Omit<Todo, 'id' | 'completed' | 'createdAt' | 'completedAt'>) => {
    const newTodo: Todo = {
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <Box sx={{ maxWidth: 1200, margin: '0 auto', p: 3 }}>
        <AppBar position="static" color="default" sx={{ borderRadius: 1, mb: 3 }}>
          <Tabs
            value={activeView}
                ...
                <Paper sx={{ p: 3, mb: 2 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                    <Chip
                      ...
                      sx={{ 
                        pl: 0.5,
                        '& .MuiChip-label': { fontWeight: selectedType === 'all' ? 500 : 400 }
                      }}
                      icon={<Box sx={{ 
                        width: 8,
                        height: 8,
                        bgcolor: selectedType === 'all' ? 'white' : 'primary.main',
                        borderRadius: '50%',
                        ml: 1
                      }} />}
                    />
                    {Object.entries(taskColors).map(([type, color]) => (
                      <Chip
                        key={type}
                        label={type === 'home' ? 'Home tasks' : 
                               type === 'kids' ? 'Kids tasks' : 
                               type === 'work' ? 'Work tasks' : 
                               type.charAt(0).toUpperCase() + type.slice(1)}
                        onClick={() => setSelectedType(type as TaskType)}
                        variant={selectedType === type ? 'filled' : 'outlined'}
                        sx={{
                          backgroundColor: type !== 'all' ? taskColors[type as TaskType] : 'transparent',
                        ...
                        secondary={
                          <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', mt: 0.5 }}>
                            <Chip
                              label={todo.type}
                              size="small"
                              sx={{ bgcolor: taskColors[todo.type] + '20', color: taskColors[todo.type] }}
                            />
                            <Chip
                            ...
                            <Chip
                              ...
                              sx={{
                                ...
                                color: todo.priority === 'high' ? '#993333' : todo.priority === 'medium' ? '#FF9966' : '#66CC99',
                              }}
                              variant="outlined"
                            />
                          </Box>
